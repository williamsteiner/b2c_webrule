/*global history */
sap.ui.define([
	"com/owi/b2c/b2cWebRule/controller/BaseController",
	"sap/ui/model/json/JSONModel",
	"sap/ui/model/Filter",
	"sap/ui/model/FilterOperator",
	"sap/m/GroupHeaderListItem",
	"sap/ui/Device",
	"com/owi/b2c/b2cWebRule/model/formatter",
	"sap/m/MessageBox"
], function (BaseController, JSONModel, Filter, FilterOperator, GroupHeaderListItem, Device, formatter, MessageBox) {
	"use strict";

	return BaseController.extend("com.owi.b2c.b2cWebRule.controller.Master", {
		...
		onSearch: function (oEvent) {
			...
			if (sQuery) {
				this._oListFilterState.aSearch = [new Filter("RULENAME", FilterOperator.Contains, sQuery)];
			} else {
				...
				function (mParams) {
					...
					if (!mParams.list.getSelectedItem()) {
						this.getRouter().navTo("object", {
							RULETYPE: encodeURIComponent(mParams.firstListitem.getBindingContext().getProperty("RULETYPE")),
							RULEID: encodeURIComponent(mParams.firstListitem.getBindingContext().getProperty("RULEID")),
							RULENAME: encodeURIComponent(mParams.firstListitem.getBindingContext().getProperty("RULENAME"))
						}, true);
					}
				}.bind(this),
				function (mParams) {
		...
		_showDetail: function (oItem) {
			...
			this.getRouter().navTo("object", {
				RULETYPE: encodeURIComponent(oItem.getBindingContext().getProperty("RULETYPE")),
				RULEID: encodeURIComponent(oItem.getBindingContext().getProperty("RULEID")),
				RULENAME: encodeURIComponent(oItem.getBindingContext().getProperty("RULENAME"))
			}, bReplace);
		},
